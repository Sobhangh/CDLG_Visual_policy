...
class DistillDataset(Dataset):
	def __init__(self, dataset_dir: str):
		...
		self.obs = np.memmap(
			os.path.join(dataset_dir, "observations.uint8.mmap"),
			mode="r",
			dtype=np.uint8,
			shape=(self.n_samples, 4, 84, 84),
		)
		self.logits = np.memmap(
			os.path.join(dataset_dir, "teacher_logits.float16.mmap"),
			mode="r",
			dtype=np.float16,
	...
	def __getitem__(self, idx: int):
		obs = torch.from_numpy(np.asarray(self.obs[idx], dtype=np.uint8))
		logits = torch.from_numpy(np.asarray(self.logits[idx], dtype=np.float32))
		return obs, logits
...
class ShardDistillDataset(Dataset):
	def __init__(self, shard_pairs: list[tuple[str, str]]):
		...
		for obs_path, logits_path in shard_pairs:
			...
			self.logit_arrays.append(logits_arr)
	...
	def __getitem__(self, idx: int):
		...
		local_idx = idx - start
		obs = torch.from_numpy(np.asarray(self.obs_arrays[shard_idx][local_idx], dtype=np.uint8))
		logits = torch.from_numpy(np.asarray(self.logit_arrays[shard_idx][local_idx], dtype=np.float32))
		return obs, logits
