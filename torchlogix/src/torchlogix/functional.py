...
def sigmoid(logits, hard=False, tau=1.0):
    ...
    if hard:
        y_hard = (y_soft > 0.5).to(dtype=y_soft.dtype)
        return (y_hard - y_soft).detach() + y_soft
    return y_soft
...
def walsh_hadamard_transform(x: torch.Tensor, n: int, dtype=torch.float32, 
    ...
    if fast:
        ...
        # Keep integer inputs in float32 for matmul support, but preserve
        # floating compute dtypes (e.g., float16/bfloat16) when provided.
        if device is None:
            device = x.device
        compute_dtype = x.dtype if x.is_floating_point() else torch.float32
        H = hadamard_matrix(1 << n, dtype=compute_dtype, device=device)
