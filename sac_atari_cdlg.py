...
try:
    from torchlogix.layers import Binarization, FixedBinarization, LogicConv2d, LogicDense, OrPooling2d, GroupSum
except ImportError:
    Binarization = None
...
def evaluate(
    ...
    while len(episodic_returns) < eval_episodes:
        with torch.no_grad():
            actions, _, _ = agent.get_action(torch.as_tensor(obs, dtype=torch.float32, device=device))
...
def build_logic_backbone(
    *,
    ...
    """Build the CDLG convolutional encoder used by the SAC actor and critics."""
...
class LogicSoftQNetwork(nn.Module):
    def __init__(self, envs, args: Args, thresholds: torch.Tensor):
        ...
        self.backbone, backbone_out_dim = build_logic_backbone(
            h=h,
            k=k,
            c0=c0,
        ...
        self.fc1 = layer_init(nn.Linear(backbone_out_dim, 512))
...
if __name__ == "__main__":
    ...
    if args.agent_arch.lower() == "cdlgnn":
        ...
        thresholds = get_distributive_channel_thresholds(calibration_obs=calibration_obs, num_bits=args.logic_num_bits)
        actor = LogicActor(envs, args=args, thresholds=thresholds).to(device)
    else:
